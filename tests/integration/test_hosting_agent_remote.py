...
async def test_remote_hosting_agent():
    print("\n=== Testing Remote Hosting Agent with A2A Client ===\n")

    # Configuration
    project_id = test_config.PROJECT_ID
    location = test_config.LOCATION
    project_number = test_config.PROJECT_NUMBER
    hosting_agent_id = test_config.HOSTING_AGENT_ID
    ...
    # Poll for result
    print("\nPolling for result...")
    for i in range(60):  # Poll up to 60 times (2 minutes)
        ...
        if response.status.state == "TASK_STATE_COMPLETED":
            if hasattr(response, "artifacts") and response.artifacts:
                ...
                print("Task completed but no artifacts found")
                return False
...
if __name__ == "__main__":
    success = asyncio.run(test_remote_hosting_agent())
